...
(function( bc, undefined ) {
   ...
   * @param event
   */
  bc.ga.track = function( category, action, options ) {
    ...
    src += "&utmhn=www.metoperafamily.org"; // host name
    src += "&utmt=event"; // we're tracking an event
    src += "&utme=" + createEventString( category, action, options ); // the event data
    ...
    src += "&utmhid=124093591"; // random number to link to adsense
  ...
  function createEventString( category, action, options ) {
    ...
    if( options.label ) {
      str += "*" + encodeURIComponent( options.label );
    }
    str += ")";
    if( options.value && typeof options.value === "number" ) {
      str += "(" + encodeURIComponent( options.value ) + ")";
    }
